public class ItemSistema {
    public String nome;
    public int complexidade;
    public int complexidadeEntradaSaidaDados;
    public int complexidadeConsultaDados;
    public int complexidadeGravarDados;

    public int CalculaEsforçoHoras() {
        //1 4(1 = 24, 2 = 48, 3 = 72, 4 = 96)
        switch (complexidade) {
            case 1:
                return 24;
            case 2:
                return 48;
            case 3:
                return 72;
            case 4:
                return 96;
            default:
                return -1;
        }
    }

    public double CalculaEsforçoEntradaSaidaDados() {
        return 0;
    }

    public double CalculaEsforçoConsultaDados() {
        return 0;
    }

    public double CalculaEsforçoGravarDados() {
        return 0;
    }


    
}